...
export default function Sidebar({ isOpen, onClose }) {
    ...
    const sidebarContent = (
        <div className="h-full w-64 bg-white border-r border-gray-200 flex flex-col">
            {/* Logo */}
            <div
                onClick={() => router.push('/falcon')}
                className="p-6 border-b border-gray-200 flex justify-center cursor-pointer"
            >
                <Image
            ...
            `}
                    >
                        ...
                        <ChevronDown
                            className={`w-4 h-4 transition-transform duration-200 ${isFalconExpanded ? 'rotate-180' : ''}`}
                        />
                    </button>
                    ...
                    {isFalconExpanded && (
                        <div className="mt-1 ml-4 pl-4 border-l-2 border-gray-100 space-y-1">
                            {falconSubItems.map((item) => {
                                ...
                                const isSubActive =
                                    isFalconActive && currentTab === item.id
